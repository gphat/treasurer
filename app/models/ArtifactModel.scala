...
object ArtifactModel {
  ...
  val allQuery = SQL("SELECT * FROM artifacts")
  val getByIdQuery = SQL("SELECT * FROM artifacts WHERE id={id} AND project_id={project_id}")
  val insertQuery = SQL("INSERT INTO artifacts (id, project_id, version, date_created, date_internal) VALUES ({id}, {project_id}, {version}, {date_created}, {date_internal})")
  ...
  val artifact = {
    ...
    get[DateTime]("date_internal") map {
      case id~version~dateCreated~dateInternal => Artifact(
        id = id.get,
        version = version,
   ...
   * Get all artifacts.
   */
  def getAll: List[Artifact] = {

    DB.withConnection { implicit conn =>
      allQuery.as(artifact.*)
    }
  }
